...
import { RecurringTransactionPayload } from "../types/recurringTransaction";
...
interface AddRecurringModalProps {
  ...
  categories: any[];
  accounts: any[];
  onSubmit: (payload: RecurringTransactionPayload) => Promise<void>;
  defaultValues?: Partial<RecurringTransactionPayload>;
  mode?: "create" | "edit";
}

export default function AddRecurringModal(props: AddRecurringModalProps) {
  const {
    open,
    onOpenChange,
    categories,
  ...
  // Form State
  // ----------------------------
  // Swagger DTO에 title이 없으므로 description을 이름으로 사용합니다.
  ...
  // 주기 설정
  const [repeatType, setRepeatType] = useState<"MONTHLY" | "WEEKLY">(
    defaultValues?.repeatType ?? "MONTHLY"
  );
  const [repeatDay, setRepeatDay] = useState<string>(
    defaultValues?.repeatDay ? String(defaultValues.repeatDay) : "1"
  );
  ...
  useEffect(() => {
    ...
    setRepeatType(defaultValues?.repeatType ?? "MONTHLY");
    setRepeatDay(defaultValues?.repeatDay ? String(defaultValues.repeatDay) : "1");
    setStartDate(defaultValues?.startDate ?? new Date().toISOString().split("T")[0]);

    setError("");
    setSubCatError("");
  }, [open, defaultValues]);

  // 주기 타입이 바뀔 때 요일/날짜 값 보정 (필요 시)
  useEffect(() => {
    if (repeatType === "WEEKLY" && Number(repeatDay) > 7) {
      setRepeatDay("1");
    }
  }, [repeatType]);
    ...
    <>
      {open && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
          ...
          <div className="relative w-full sm:max-w-xl mx-auto bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl p-6 pb-safe sm:pb-6 space-y-5 animate-in slide-in-from-bottom-4 sm:slide-in-from-bottom-0 sm:fade-in-0 duration-200 max-h-[90dvh] sm:max-h-[90vh] overflow-y-auto">
            {/* 헤더 */}
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold text-gray-800">
                {mode === "edit" ? "반복 거래 수정" : "새 반복 거래 추가"}
              </h2>
              <button
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                <Label htmlFor="description" className="ml-1">
                  이름 (내용) <span className="text-red-500">*</span>
                </Label>
                <Input
                  id="description"
                  placeholder="예: 넷플릭스 구독 (최대 15자)"
                  value={description}
              ...
              {/* 반복 주기 설정 */}
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 p-3 bg-slate-50 border rounded-xl">
                <div className="space-y-2">
                  <Label className="ml-1">반복 주기</Label>
                  <Select
                    value={repeatType}
                    onValueChange={(v) => setRepeatType(v as "MONTHLY" | "WEEKLY")}
                  >
                ...
                <div className="space-y-2">
                  {repeatType === "MONTHLY" ? (
                    <>
                      <Label className="ml-1">결제일</Label>
                      <Select value={repeatDay} onValueChange={setRepeatDay}>
                        <SelectTrigger className="w-full bg-white">
                          <SelectValue placeholder="선택" />
                        </SelectTrigger>
                    ...
                    <>
                      <Label className="ml-1">결제 요일</Label>
                      <Select value={repeatDay} onValueChange={setRepeatDay}>
                        <SelectTrigger className="w-full bg-white">
                          <SelectValue placeholder="선택" />
                        </SelectTrigger>
              ...
              {/* 시작일 (Swagger에 필수 필드로 정의됨) */}
              <div className="space-y-2">
                <Label htmlFor="startDate" className="ml-1">
