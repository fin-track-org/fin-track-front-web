...
export default function ProfileAccountTab() {
  ...
  const providerLabel = (provider: string) => (provider === "kakao" ? "카카오" : "구글");

  const handleLink = async (provider: "kakao" | "google") => {
    try {
      ...
      toast.error(
        message.includes("already linked")
          ? `이 ${providerLabel(provider)} 계정은 이미 다른 가계부와 연동되어 있습니다.`
          : `연동 실패: ${message}`
      );
      setIsLinking(null);
    }
  ...
  const handleUnlink = async (provider: "kakao" | "google") => {
    try {
      ...
      if (isOnlyProvider) {
        toast.error(`현재 ${providerLabel(provider)} 계정으로만 로그인되어 있습니다. 연동을 해제하시려면 화면 하단의 [회원 탈퇴]를 이용해 주세요.`);
        return;
      }

      const targetIdentity = identities.find((id) => id.provider === provider);
      if (!targetIdentity) {
        toast.error(`연결된 ${providerLabel(provider)} 계정이 없습니다.`);
        return;
      }
      ...
      const updated = await updateMe({ linkedProviders, availableAvatars });
      queryClient.setQueryData(["me"], updated);
      toast.success(`${providerLabel(provider)} 연동이 해제되었습니다.`);
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : "알 수 없는 오류";
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between p-3 rounded-xl border border-gray-100 bg-gray-50/50">
                ...
                {data.linkedProviders?.includes('kakao') ? (
                  ...
                  <button
                    ...
                    className="text-xs font-semibold text-[#371d1e] bg-[#FEE500] hover:bg-[#FDD800] disabled:opacity-50 px-3.5 py-1.5 rounded-lg transition-colors shadow-sm"
                  >
              ...
              <div className="flex items-center justify-between p-3 rounded-xl border border-gray-100 bg-gray-50/50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-800">구글 연동</p>
                    <p className="text-xs text-gray-500 mt-0.5">
                      {data.linkedProviders?.includes('google') ? "구글 계정으로 안전하게 로그인 중입니다." : "클릭하여 구글 계정과 연동하세요."}
                    </p>
                  </div>
                </div>
                {data.linkedProviders?.includes('google') ? (
                  ...
                  <button
                    ...
                    className="text-xs font-semibold text-gray-700 border border-gray-200 bg-white hover:bg-gray-50 disabled:opacity-50 px-3.5 py-1.5 rounded-lg transition-colors shadow-sm"
                  >
