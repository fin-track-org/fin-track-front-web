  ...
  const handlePing = async () => {
    ...
    try {
      ...
      if (error.name === "TypeError" && error.message.includes("fetch")) {
        setPingResult(
          "❌ 실패: CORS 정책 위반 또는 네트워크 에러. (백엔드 CORS 설정을 확인하세요!)",
        );
      } else {
        ...
        {(pingResult?.includes("CORS") || pingDbResult?.includes("CORS")) && (
          <div className="mt-6 p-4 bg-yellow-100 text-yellow-800 rounded-lg">
            ...
            <p className="text-sm mt-1">
              Spring Boot 컨트롤러에{" "}
              <strong>`@CrossOrigin(&quot;http://localhost:3000&quot;)`</strong>{" "}
              어노테이션을 추가하거나, 글로벌 <strong>`WebConfig`</strong>{" "}
              파일을 만들어 `localhost:3000`을 허용해야 합니다.
            </p>
          </div>
        )}
      </div>
