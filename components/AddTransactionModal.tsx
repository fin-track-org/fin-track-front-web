...
export default function AddTransactionModal({ onClose, onSaveSuccess, currentTransaction }: AddTransactionModalProps) {
    const supabase = createClient();
    
    // (A) 초기 상태 설정: currentTransaction이 있으면 수정 모드로 초기화
    const initialAmount = currentTransaction ? Math.abs(currentTransaction.amount) : 0;
    const initialIsExpense = currentTransaction ? currentTransaction.amount < 0 : true;

    const [date, setDate] = useState(currentTransaction?.date || new Date().toISOString().split('T')[0]);
    const [amount, setAmount] = useState(initialAmount);
    const [category, setCategory] = useState(currentTransaction?.category || '식비');
        ...
        >
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex rounded-md shadow-sm">
                        <button
                            ...
                            className={`flex-1 px-4 py-2 rounded-l-md transition-colors ${
                                isExpense ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                            }`}
                            disabled={loading}
                        >
                            출금 (지출)
                        ...
                        <button
                            ...
                            className={`flex-1 px-4 py-2 rounded-r-md transition-colors ${
                                !isExpense ? 'bg-sky-500 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                            }`}
                            disabled={loading}
                        >
                            입금 (수입)
                    ...
                    {/* 금액 */}
                    <div>
                        <label htmlFor="amount" className="block text-sm font-medium text-gray-700">금액 (원)</label>
                        <input
                            id="amount"
                            type="number"
                    ...
                    {/* 내역 */}
                    <div>
                        <label htmlFor="description" className="block text-sm font-medium text-gray-700">내역</label>
                        <input
                            id="description"
                            type="text"
                            value={description}
